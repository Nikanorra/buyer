<template>
  <div class="v-autocomplete">
    ...
    <div class="row">
      <div class="btn-orange" @click="addAddress(selectType)">Save changes</div>
      <div @click="cancelEdit" class="btn-light">Cancel</div>
    </div>
  </div>
</template>
...
export default {
  props: ["locations", "addresses", "selectStreet", "form",],
  data() {
    return {
      value: "",
      selectType: "",
      focused: false,
      newAddress: this.locations
    };
  },
  computed: {
    searchHandler() {
      return this.newAddress.filter((elem) => {
  ...
  methods: {
    async enterAddress(value) {
      if (value.length >= 3) {
        ...
        const res = await axios.get(`${process.env.VUE_APP_HOST}/system/addresses?address=${addressString}`)
    ...
    async addAddress(selectType) {
      this.$emit("saveAddress", selectType);
    }
  },
};
</script>
